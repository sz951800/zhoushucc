d3.json("data2.json").then(function (data) {
    ...
    const countries = [...new Set(data.map(d => d["Country or Area"]))];
    ...
    const yScale = d3
      .scaleLinear()
      .domain([30000, 80000]) // Adjust the domain based on your data
      .nice()
    ...
    const lines = svg
      ...
      .attr("d", function (country, i) {
        const countryData = data.filter(
          d => d["Country or Area"] === country && years.includes(d.Year)
        );
    ...
    svg
      ...
      .text("GDP per Capita Comparison (by year)");
